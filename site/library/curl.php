...
class Curl{
     ...
     *
     * @param string $url :请求的URL
     * @param string|array $params ：请求的参数,格式如: array('id'=>10,'name'=>'yuanwei')或者为string
     ...
     * @return 错误返回:false 正确返回:结果内容
     */
    public function post($url,$params,$uploadFile=array(), $referer=''){
     ...
     * @return 错误返回:false 正确返回:结果内容
     */
    private function _request($method, $url, $params, $uploadFile=array(), $referer=''){
        ...
        //如果是POST
        if($method == 'POST'){
            //发送一个常规的POST请求,类型为：multipart/form-data
            curl_setopt($this->ch, CURLOPT_POST, true) ;
            $postData =(is_array($params)||$uploadFile)? $this->_parsmEncode($params,false):$params;
            //如果有上传文件
            if($uploadFile){
    ...
    private function _parsmEncode($params,$isRetStr=true){
        ...
        foreach($params as $key=>$value){
            $fieldStr .= $spr.$key .'='.urlencode($value);
            $spr = '&';
            $result[$key] = $value;
        }
