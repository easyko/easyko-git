...
class ProjectController extends CommonController
{
    ...
    public function __construct($config = array())
    {
        ...
        // 文件上传临时目录
        $this->modelUpload->setUploadTempDirName($uploadTempDirName);
    ...
    public function create()
    {
        ...
        if ($projectName == '' || $customerName == '' || $startDate == '' || $endDate == ''
            || $managerId == '0' || $projectDesc == '' || $projectNo == '') {
    ...
    private function moveFile($projectNo)
    {
        ...
        if (!empty($tempList)) {
            foreach ($tempList as $temp) {
                ...
                $pos = strpos($attach, $this->modelUpload->getUploadTempDirName());
                $tempFile = substr($attach, $pos + strlen($this->modelUpload->getUploadTempDirName()));
                $path = $this->rootDir . $this->modelUpload->getUploadTempDirName() . $tempFile;
                ...
                if (rename($path, $newPath)) {
                    $obj = array(
                        'status' => 1,
                        'file_url' => str_replace($this->modelUpload->getUploadTempDirName(), '', $temp['file_url'])
                    );
