...
class WeeklyController extends CommonController
{
	...
	public function index()
	{
		...
		$model = $this->createModel('Model_Weekly', dirname( __FILE__ ));
	...
	public function upload()
	{
		...
		if ($method == "POST") {
			...
			if ($result["success"]) {
				...
				if (!$model->store('weekly_report', $object)) {
	...
	function del()
	{
		$weeklyId  = intval(Fuse_Request::getVar('weekly_id'));
		$userId    = intval(Fuse_Request::getVar('user_id'));
		$startDate = Fuse_Request::getVar('start_date');
		$endDate   = Fuse_Request::getVar('end_date');

		$model = $this->createModel('Model_Weekly', dirname( __FILE__ ));
		$list = $model->getWeeklyById($weeklyId);
		if (empty($list)) {
			die(json_encode(array('code'=> '1111', 'message' => '参数错误，删除失败', 'data' => '')));
		}

		$model->delWeeklyById($weeklyId);
