 ...
 * $date    2016-06-15 21:01:45
 */

class Model_ProjectReport extends Fuse_Model
{

	private $table     = array('name' => 'ek_project_report', 'key' => 'project_report_id');
	...
	{
		$list = array();
		$sql = "SELECT w.project_report_id AS projectReportId,w.name AS fileName,
					w.time AS date,tu.`username` AS managerName
		...
		if ($stmt = $this->db->query($sql)) {
			while ($row = $stmt->fetch()) {
				...
				// 格式化
				$row['date'] = str_replace('-', '/', substr($row['date'], 0, 10));

				// $row['name_url'] = '/work_summary/'. $row['user_id'] . '/' . $row['name'];
	...
	public function getReportById($reportId)
	{
		...
		$sql = "SELECT * FROM `{$this->table['name']}` WHERE `{$this->table['key']}` = '{$reportId}' LIMIT 1";
	...
	public function delReportById($weeklyId)
	{
		$list = array();
