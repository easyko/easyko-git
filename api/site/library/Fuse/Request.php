...
class Fuse_Request
{
	...
	/**
	 * @param $name
	 * @param $type 0:默认 1:转数字
	 * 
	 */
	public static function getFormatVar($name, $key, $type = '0')
	{
		$value = isset($name['params'][$key]) ? trim($name['params'][$key]) : '';
		if ($type == '1' && $value != '') {
			$value = intval($value);
		}
		
		if ($value == '') {
			$value = isset($name[$key]) ? trim($name[$key]) : '';
			if ($type == '1' && $value != '') {
				$value = intval($value);
			}
		}
